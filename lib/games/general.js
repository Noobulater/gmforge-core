...
exports.commands["registerTag"] = function(gameID, data, userID, override) {
  var game = gameControls.getGame(gameID);
  if (!game || !data || !data.tag) {return false;}

  game.templates.tags = game.templates.tags || {};
  if (game.templates.tags[data.tag.toLowerCase()]) {
    if (gameControls.security(gameID, userID, 2)) {
      game.templates.tags[data.tag.toLowerCase()] = data.tagData;
      var update = {
        header : {
          type : "tags",
        },
        data : game.templates.tags
      };
      gameControls.broadcast(gameID, update);
    }
    else {
      users.alert(userID, {text : "A tag already exists with this name"});
    }
  }
  else if (gameControls.security(gameID, userID, 3)) {
    game.templates.tags[data.tag.toLowerCase()] = data.tagData;
    var update = {
      header : {
        type : "tags",
    ...
    users.alert(userID, {text : "You must be a Trusted Player to create your own tags"});
  }
}

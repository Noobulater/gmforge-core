...
var gameCommands = {};
gameCommands["general"] = require("./general.js");
...
function createGame(gameData, gameName, gameKey, userID) {
  if (!gameData) {
    ...
    gameCommands["general"].commands["logEvent"](gameName, {text : "/w /w <text> - whispers", style : {"background-color" : "rgba(66,108,66,0.2)", "font-size" : "0.9em"}}, null, true);
    gameCommands["general"].commands["logEvent"](gameName, {text : "/me /me <text> - perform an action!", style : {"background-color" : "rgba(255,138,0,0.2)"}}, null, true);
    ...
    try {
      ...
      games["localhost"] = JSON.parse(gameData.substring(1, gameData.length));
    }
...
function saveGame(gameID, sync) {
  if (games["localhost"]) {
    var game = games["localhost"];
    var buf = new Buffer(JSON.stringify(game, 2, 2)); // decode
    if (game.config.name) {
      if (sync) {
        fs.writeFileSync('./public/worlds/' + game.config.name, buf);
      }
      else {
        fs.writeFile('./public/worlds/' + game.config.name, buf, function(err) {
          if (err) {
            console.log("err", err);
          }
...
function updatePlayers(gameID, joinID) {
  ...
  for (var playerID in game.players) {
    ...
    sendData[playerID].displayName = game.players[playerID].displayName || playerID;
...
function joinGame(gameID, userID) {
  ...
  var game = games["localhost"];
  if (game) {
    game.config.players = options.accounts || game.config.players || {};

    game.players[userID] = game.config.players[userID] || {
      name : userID,
      _t : "p",
    ...
    if (game.config.players[userID]) {
      game.players[userID].displayName = game.config.players[userID].displayName || userID;
    }
    if (userID == "localhost" && options.hostName) {
      game.players[userID].displayName = options.hostName;
    }
    game.players[userID]._t = "p";
    game.players[userID].rank = game.players[userID].rank || security.player["Trusted Player"];
    ...
    gameCommands["general"].commands["logEvent"](gameID, {text : ((game.config.players[userID] !=null)?(game.config.players[userID].displayName):(userID)) + " has connected", color : "#EBEBE4"}, userID, true);
...
function getGameTypes() {
  ...
  files.forEach(file => {
    if (!fs.lstatSync("./public/templates/"+file).isDirectory()) {
      var data = fs.readFileSync("./public/templates/"+file, 'utf8');
      try {
        var template = JSON.parse(data);
        gameDisplay[template.identifier] = template;
      }
      catch (err) {
  ...
  {
    if(!fs.existsSync("./public/workshop/"+mod+"/templates")){return;}
    fs.readdirSync("./public/workshop/"+mod+"/templates").forEach(function(file){
      if (!fs.lstatSync("./public/workshop/"+mod+"/templates/"+file).isDirectory()) {
        var data = fs.readFileSync("./public/workshop/"+mod+"/templates/"+file, 'utf8');
        try {
          var template = JSON.parse(data);
          gameDisplay[template.identifier] = template;
        }

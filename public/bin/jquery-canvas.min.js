...
function _setCursor($canvas, layer, eventType) {
	...
	if (cursor) {
		// Set canvas cursor
		$canvas.css({
			cursor: $canvas.attr("cursor") || cursor
		});
	}
}
...
function _resetCursor($canvas, data) {
	$canvas.css({
		cursor: $canvas.attr("cursor") || data.cursor
	});
}
		...
		if (ctx) {
			...
			if (params.visible) {
				...
				if (source.src || imgCtx) {
					...
					if (imageCache[source] && imageCache[source].complete && imageCache[source].naturalWidth && imageCache[source].naturalHeight) {
